...
function _load_stark_profiles(fname)
    h5open(fname, "r") do fid
        map(keys(fid)) do transition
            ...
            profile = linear_interpolation((temps, nes,
                                            [-floatmax(); log.(delta_nu_over_F0[2:end])]),
                                           logP;
...
function brackett_line_stark_profiles(m, λs, λ₀, T, nₑ)
    ...
    impact_electron_profile = map(y1, y2, βs) do y1, y2, β
        ...
        width = if (y2 <= 1e-4) && (y1 <= 1e-5)
            ...
            GAM = (G1 *
                   (0.5 * exp(-min(80, y1)) + exponential_integral_1(y1) -
                    0.5 * exponential_integral_1(y2)) *
...
const _greim_Kmn_table = [0.0001716 0.0090190 0.1001000 0.5820000
...
const _holtsmark_PP = [0.0, 0.2, 0.4, 0.6, 0.8]
const _holtsmark_β_knots = [1.0, 1.259, 1.585, 1.995, 2.512, 3.162, 3.981,
