...
function implicit_jacobian_diff(residuals_constructor, x_sol, p...)
    ...
    vp = map(p) do x
        x isa AbstractArray ? ForwardDiff.value.(x) : ForwardDiff.value(x)
    end

    # evaluate the residuals at x_sol with dual parameters to get the partials: ∂F = ∂R/∂p · dp
    ...
    # Compute ∂R/∂x at the solution
    tmp = similar(x_sol)

    ∂R_∂x = ForwardDiff.jacobian((tmp, x) -> residuals_constructor(p...)(tmp, x), tmp,
                                 eltype(F_dual).(x_sol))

    # Solve ∂x/∂p · ∂p = −(∂R/∂x)⁻¹ (∂R/∂p · ∂p) 
    ∂x = -(∂R_∂x \ ∂F)
