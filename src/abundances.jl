...
You can specify abundances with these positional arguments. All are optional, but if
`default_alpha_H` is provided, `default_metals_H` must be as well.

  - `default_metals_H` (default: 0): [metals/H] is the ``\\log_{10}`` solar-relative abundance of elements heavier
    than He. It is overridden by `default_alpha_H` and `abundances` on a per-element basis.
  - `default_alpha_H` (default: same as `default_metals_H`): [alpha/H] is the ``\\log_{10}``
  ...
  - `abundances`: A `Dict` mapping atomic numbers or symbols to [``X``/H] abundances. (Set
    `solar_relative=false` to use ``A(X)`` abundances instead.) These override `default_metals_H`.
    This is the only way to specify a non-solar abundance of He.
...
# Keyword arguments
  ...
  - `solar_relative` (default: true): When true, interpret abundances as being in \\[``X``/H\\]
    ...
    Note that abundances not explicitly specified still depend on the solar value, as
    they are set according to `default_metals_H` and `default_alpha_H`.
  - `solar_abundances` (default: `Korg.bergemann_2025_solar_abundances`): The set of solar abundances to
    ...
    for (el, abund) in abundances
        ...
        if el isa AbstractString
            ...
            elseif Korg.atomic_numbers[el] in keys(abundances)
                throw(ArgumentError("The abundance of $el was specified by both atomic number and atomic symbol."))
            else
...
# Keyword Arguments
  ...
  - `solar_abundances` (default: `Korg.bergemann_2025_solar_abundances`): The set of solar abundances to
    ...
    convenience.
  - `ignore_alpha` (default: `true`): Whether to ignore the alpha elements when calculating
